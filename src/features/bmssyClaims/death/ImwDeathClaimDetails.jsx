...
const ImwDeathClaimDetails = ({ id }) => {
    ...
    const { data: duplicateAadhaarData } = useQuery(
        ["alc-imw-claim-check-duplicate-aadhaar", data?.basicDetails?.ssin_no, data?.nomineeDetails?.nominee_aadhaar],
        () => fetcher(`/alc-imw-claim-check-duplicate-aadhaar?ssin_no=${data?.basicDetails?.ssin_no}&aadhaar=${data?.nomineeDetails?.nominee_aadhaar}`),
        {
            enabled: data ? true : false,
        }
    );
        ...
        <>
            ...
            {data && (
                <div>
                    ...
                    {data?.basicDetails?.status.trim() != "A" && <ErrorAlert error={{ message: "Beneficiary Not Active" }} />}
                    ...
                    <div className="card datatable-box shadow mb-2">
                        ...
                        <div className="card-body">
                            <div className="details_section">
                                ...
                                <div>
                                    <b>Date Of Death : </b> <span className="d-inline-flex px-2 fw-semibold text-success bg-success bg-opacity-10 border border-success border-opacity-10 rounded-2">{data?.nomineeDetails?.date_of_death}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="card datatable-box shadow mb-2">
                        <div className="card-header py-2">Documents Details</div>
                        <div className="card-body">
                    ...
                    {(parseInt(data?.basicDetails?.submit_status) === 1 || parseInt(data?.basicDetails?.submit_status) === 8) && data?.basicDetails?.status.trim() === "A" && (
                        <ImwClaimDetailsSubmitForm id={id} approvedShare={data?.approvedShare} claimShare={data?.nomineeDetails?.claim_share} duplicateBankDataSSIN={duplicateBankData?.ssin} aadhaarDuplicateStatus={duplicateAadhaarData?.status} />
                    )}
...
export default ImwDeathClaimDetails;
