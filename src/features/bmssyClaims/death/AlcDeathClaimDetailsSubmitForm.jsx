...
const AlcDeathClaimDetailsSubmitForm = ({ id, duplicateBankDataSSIN, SubmitStatus, status }) => {
    const [runUseQuery, setRunUseQuery] = useState(false);
    useEffect(() => {
        if (id) setRunUseQuery(true);
    }, [id]);

    const { data: checkChronological, isLoading: loadingChronological, error } = useQuery(["check-chronological-order", id, SubmitStatus], () => fetcher(`/check-chronological-order?id=${id}&status='${SubmitStatus}'`), { enabled: runUseQuery });
...
export default AlcDeathClaimDetailsSubmitForm;
