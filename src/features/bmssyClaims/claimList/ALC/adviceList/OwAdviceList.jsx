...
const OwAdviceList = ({ type }) => {
    ...
    const benefitDetailsDownloadPFNominee = async (id) => {
        try {
            setShow(id);
            const doc = await downloadFile(`/rlo-claim-advicelist-generate-excel-nominee-pf-ow?id=${id}&type=${type}`, "DEATH BENEFIT DETAILS ADVICE-" + id + ".xlsx");
