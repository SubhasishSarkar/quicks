...
const DependentDetailsNdfForm = ({ nextStep, prevStep }) => {
    const query = useQueryClient();
    const [searchParams] = useSearchParams();
    const application_id = searchParams.get("application_id");
    const { data } = useQuery(["caf-registration-preview", "dependent-details", application_id], () => fetcher(`/caf-registration-preview?id=${application_id}&step_name=dependent-details`), {
        ...disableQuery,
    ...
    const [form, validator] = useValidate({
        ...
        relationship_name: { value: "", validate: "" },
        ...
        validator.validOnChange(evt, (value, name, setState) => {
            switch (name) {
                case "relationship":
                    setState((prev) => {
                        if (value === "Other") {
                            prev.relationship_name.validate = "required";
                            prev.relationship_name.required = true;
                        } else {
                            prev.relationship_name.value = "";
                            prev.relationship_name.validate = "";
                            prev.relationship_name.required = false;
                        }
                        return { ...prev };
                    });
                    break;
            ...
            {added && (
                <div className="card mt-2" style={{ position: "relative" }}>
                    <div className="card-header">
                        <h5 className="mb-0">Dependent Details Breakdown</h5>
                    </div>
                    <div className="card-body">
...
export default DependentDetailsNdfForm;
