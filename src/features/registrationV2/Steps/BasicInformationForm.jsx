...
const BasicInformationForm = ({ workerTypeData, nextStep }) => {
    const { basicDetails, setBasicDetails, setWorkerType } = useContext(RegistrationContext);
    ...
    } = useQuery(["caf-registration-preview", "basic-details", application_id], () => fetcher(`/caf-registration-preview?id=${application_id}&step_name=basic-details`), {
        enabled: application_id ? true : false,
    });

    useEffect(() => {
        if (!isLoadingData && data && !isFetching && !basicDetails) setBasicDetails(data);
    }, [data, isLoadingData, isFetching, basicDetails, setBasicDetails]);

    const { mutate, isLoading } = useMutation(({ body, method, url }) => updater(url, { method: method || "POST", body: body }));
    const [form, validator] = useValidate(
        {
            registration_type: { value: data?.registration_type === "" ? "NEW" : data?.registration_type === "NEW" ? "NEW" : data?.registration_type === "OLD" ? "OLD" : "NEW", validate: "" },
            ...
            ssin: { value: "", validate: data?.registration_type === "OLD" ? "required|length:12|number" : "" },
            registration_no: { value: workerTypeData?.edistrict_registration_no ?? "", validate: data?.registration_type === "OLD" ? "required" : edistrict_registration_date ? "true" : "" },
            registration_date: { value: workerTypeData?.edistrict_registration_date ?? "", validate: data?.registration_type === "OLD" ? "required" : edistrict_registration_date ? "true" : "" },
            ...
            switch (name) {
                ...
                case "marital_status":
                    setState((state) => {
                        if (state.gender.value == "Female" && value == "Married") {
                            ...
                            state.husband_name.validate = "onlyAlphabets|required";
                ...
                case "dob":
                    setState((state) => {
                        ...
                        if (age < 18 || age > 60) state.dob.error = "Please provide Date of Birth (Age limit between 18 to 60)";
                        return { ...state };
                    });
    ...
    const handleBlur = (e) => {
        if (e.length > 11) {
            ...
            validator.setState((state) => {
                state.aadhar.success = null;
                state.aadhar.error = "Invalid Aadhaar";
                return {
                    ...state,
                };
            });
    ...
    //its specialy for two fields validation when registration_type = OLD need to update have warning

    useEffect(() => {
        if (!isLoadingData && data && !isFetching) {
            validator.setState((state) => {
                ...
                <div className="card-body">
                    ...
                    <div className="row">
                        <div className="col-md-3">
                            ...
                            <input
                                className={`form-control ${form.registration_type.error && "is-invalid"}`}
                                type="text"
                                value={data?.registration_type === "" ? "NEW" : data?.registration_type === "NEW" ? "NEW" : data?.registration_type === "OLD" ? "OLD" : "NEW"}
                                disabled
                                name="registration_type"
                                id="registration_type"
                            />
                        </div>
                        {form.registration_type.value === "OLD" && (
                            <div className="col-md-3">
                        ...
                        {edistrict_registration_date || form.registration_type.value === "OLD" ? (
                            <div className="col-md-3">
                                ...
                                <input
                                    ...
                                    disabled={form.registration_no.value !== null}
                        ...
                        {edistrict_registration_date || form.registration_type.value === "OLD" ? (
                            <div className="col-md-3">
                                ...
                                <input
                                    ...
                                    disabled={edistrict_registration_date ? true : false}
                                    type="date"
                                    ...
                                    min={minDate}
                                    max={maxDate}
                                    required={form.registration_type.value === "OLD"}
                                    onChange={(e) =>
                                        handleChange({
                                            name: "registration_date",
                        ...
                        <div className="col-md-3">
                            ...
                            {/* <input placeholder="Aadhaar Number" className={`form-control ${form.aadhar.error && "is-invalid"}`} type="text" value={form.aadhar.value} name="aadhar" id="aadhar" onChange={(e) => handleChange(e.currentTarget)} /> */}
