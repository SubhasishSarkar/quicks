...
const RegistrationDateUpdateForCwTw = ({ handleClose, encId, ssin, registrationDate }) => {
    const query = useQueryClient();
    const [form, validator] = useValidate({
        updated_registration_date: { value: "", validate: "required" },
        application_id: { value: encId, validate: "required" },
        ssin_no: { value: ssin, validate: "required" },
        registration_date: { value: registrationDate, validate: "required" },
    });
    useEffect(() => {
                    ...
                    setState((state) => {
                        if (value) {
                            ...
                            if (date.isValid() && date > moment(new Date("2017-04-01"))) {
                                state.updated_registration_date.error = "Please enter less then or equal to 2017-03-31";
                            } else if (date < moment(new Date("2000-12-30"))) {
                                state.updated_registration_date.error = "Please enter grater then or equal to 2001-01-01";
                            }
                        }
                        return { ...state };
                    });
    ...
    const handleSubmit = (ev) => {
        ...
        if (encId) {
            mutate(
                {
                    url: `/wrong-reg-date-updation-cw-tw`,
                    body: data,
                },
                {
...
export default RegistrationDateUpdateForCwTw;
