...
import CommissionClaimAdviceList from "./CommissionClaimAdviceList";

const workerTypeShort = ["ow", "cw", "tw"];
const monthFull = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
const CommissionClaimGenerateAdvice = ({ workerType }) => {
    ...
    const { isLoading, error, data, isFetching } = useQuery(["claim-generate-advice", workerType, searchParams], () => fetcher(`/claim-generate-advice?${searchParams}`));
    ...
    const handleSubmit = (e) => {
        ...
        mutate(
            { url: `/claim-generate-advice-post`, body: { receipt_id: radioSelect, worker_type: workerTypeShort[workerType], data: data } },
            {
                onSuccess(data) {
                    ...
                    setGenAdvice(2);
                    setShowSubmit(false);
                    queryClient.invalidateQueries(["claim-generate-advice", workerType]);
                },
                onError(error) {
                    setShowSubmit(false);
    ...
    const handleShow = async (e) => {
        ...
        newMutate(
            { url: `/claim-view-memo-detail-list/` + memo_id },
            {
                onSuccess(data) {
                    setMemoDetails(data);
                },
    ...
    return (
        <>
            ...
            {genAdvice == 2 && <CommissionClaimAdviceList workerType={workerTypeShort[workerType]} />}
        </>
    );
};

export default CommissionClaimGenerateAdvice;
