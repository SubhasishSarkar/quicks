...
const CommissionClaimAlcForm = ({ workerType }) => {
    let total_amount = 0;
    ...
    const { isLoading, error, data } = useQuery(["commission-claim-alc", workerType, searchParams], () => fetcher(`/commission-claim-alc?${searchParams}`));
        ...
        mutate(
            { url: `/commission-claim-payment-post`, body: { checkboxSelect: checkboxSelect, worker_type: workerTypeShort[workerType] } },
            {
                onSuccess(data) {
                    ...
                    queryClient.invalidateQueries(["commission-claim-alc", workerType]);
                },
                onError(error) {
...
export default CommissionClaimAlcForm;
