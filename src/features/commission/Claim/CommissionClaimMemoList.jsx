...
const CommissionClaimMemoList = ({ workerType }) => {
    const [searchParams, setSearchParams] = useState(`workerType=${workerTypeShort[workerType]}`);
    const { isLoading, error, data } = useQuery(["claim-view-memo-list", workerType, searchParams], () => fetcher(`/claim-view-memo-list?${searchParams}`));
    ...
    const DownloadFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/claim-payment-memo-excel/" + receipt_id, "Bank Advice.xlsx");
    ...
    const DownloadOrderFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/claim-payment-order-to-rlo-excel/" + receipt_id, "Order To RLO.xlsx");
            if (docData === false) toast.error("Unable to download excel");
...
export default CommissionClaimMemoList;
