...
const CommissionGovGrantMemoList = ({ workerType }) => {
    const [searchParams, setSearchParams] = useState(`workerType=${workerTypeShort[workerType]}`);
    const { isLoading, error, data } = useQuery(["ds-view-memo-list", workerType, searchParams], () => fetcher(`/ds-view-memo-list?${searchParams}`));
    ...
    const DownloadFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/ds-gov-grant-memo-excel/" + receipt_id, "Bank Advice.xlsx");
    ...
    const DownloadOrderFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/ds-gov-grant-order-to-rlo-excel/" + receipt_id, "Order To RLO.xlsx");
            if (docData === false) toast.error("Unable to download excel");
...
export default CommissionGovGrantMemoList;
