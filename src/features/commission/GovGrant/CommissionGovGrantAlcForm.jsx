...
const statusTypes = { V: "Pending", F: "Pending at Board", C: "Pending at CF&CAO", A: "Approved by CF&CAO", M: "Fund Released by Board", 1: "Fund Released" };

const CommissionGovGrantAlcForm = ({ workerType }) => {
    let total_amount = 0;
    ...
    const [form, validator] = useValidate({
        ...
        status: { value: "V", validate: "" },
        workerType: { value: workerTypeShort[workerType], validate: "" },
    });

    const { isLoading, error, data } = useQuery(["commission-gov-grant-alc", workerType, searchParams], () => fetcher(`/commission-gov-grant-alc?${searchParams}`));
        ...
        mutate(
            { url: `/commission-gov-grant-post`, body: { checkboxSelect: checkboxSelect, worker_type: workerTypeShort[workerType] } },
            {
                onSuccess(data) {
                    ...
                    validator.reset();
                    queryClient.invalidateQueries(["commission-gov-grant-alc", workerType]);
                },
                onError(error) {
                                    ...
                                    {data?.data_Set?.data?.map((item, index) => {
                                        return (
                                            <tr key={index} data-enc-id={item.enc_application_id}>
                                                <td style={wrapStyle}>
                                                    {data.sel_status == "V" && <CheckBox value={item.id} />}
                                                    {data.sel_status != "V" && index + 1}
                                                </td>
                                                <td style={wrapStyle}>
                                                    {monthFull[item.month - 1]}-{item.year}
                                                </td>
...
export default CommissionGovGrantAlcForm;
