...
const CommissionRegistrationAdviceList = ({ workerType }) => {
    const [searchParams, setSearchParams] = useSearchParams(`worker_type=${workerType}`);
    const { isLoading, error, data } = useQuery(["view-advice-list", searchParams.toString()], () => fetcher(`/view-advice-list?${searchParams.toString()}`));
    ...
    const DownloadPaymentFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/monthly-payment-adv-details-excel/" + receipt_id, "PAYMENT DETAILS.xlsx");
    ...
    const DownloadAdviceFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/monthly-payment-adv-excel/" + receipt_id + "/1", "BANK ADVICE.xlsx");
            if (docData.status === false) toast.error("Unable to download excel");
    ...
    const DownloadTdsFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/monthly-payment-advTds-excel/" + receipt_id + "/2", "BANK ADVICE WITH TDS.xlsx");
            if (docData.status === false) toast.error("Unable to download excel");
            setOrderbankLoading();
        } catch (error) {
            toast.error(error.message);
...
export default CommissionRegistrationAdviceList;
