...
const CommissionRegistrationAlcForm = ({ workerType }) => {
    let total_amount = 0;
    ...
    const { isLoading, error, data } = useQuery(["commission-registration-alc", workerType, searchParams], () => fetcher(`/commission-registration-alc?${searchParams}`));
        ...
        mutate(
            { url: `/commission-monthly-payment-post`, body: { checkboxSelect: checkboxSelect, worker_type: workerTypeShort[workerType] } },
            {
                onSuccess(data) {
                    ...
                    validator.reset();
                    queryClient.invalidateQueries(["commission-registration-alc", workerType]);
                },
                onError(error) {
...
export default CommissionRegistrationAlcForm;
