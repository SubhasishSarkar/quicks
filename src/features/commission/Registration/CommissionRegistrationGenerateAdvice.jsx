...
import CommissionRegistrationAdviceList from "./CommissionRegistrationAdviceList";
import ErrorAlert from "../../../components/list/ErrorAlert";

const workerTypeShort = ["ow", "cw", "tw"];
const monthFull = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
const CommissionRegistrationGenerateAdvice = ({ workerType }) => {
    ...
    const { isLoading, error, data, isFetching } = useQuery(["generate-advice", workerType, searchParams], () => fetcher(`/generate-advice?${searchParams}`));
    ...
    const handleSubmit = (e) => {
        ...
        mutate(
            { url: `/generate-advice-post`, body: { receipt_id: radioSelect, worker_type: workerTypeShort[workerType], data: data } },
            {
                onSuccess(data) {
                    ...
                    setGenAdvice(2);
                    setShowSubmit(false);
                    queryClient.invalidateQueries(["generate-advice", workerType]);
                },
                onError(error) {
                    setShowSubmit(false);
    ...
    const handleShow = async (e) => {
        ...
        newMutate(
            { url: `/view-memo-detail-list/` + memo_id },
            {
                onSuccess(data) {
                    setMemoDetails(data);
                },
        ...
        <>
                                        ...
                                        {data?.data_Set?.map((item, index) => {
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td style={wrapStyle}>{item.total}</td>
                                                    <td style={wrapStyle}>
                                                        <Button variant="primary" onClick={handleShow} attrMemo={item.memo_id} attrMemoName={item.memo_no} className="me-2">
                                                            Details
                                                        </Button>
                                                    </td>
                                                </tr>
            ...
            <Offcanvas show={show} onHide={handleClose} placement="end" backdrop="static" style={{ width: "70%" }}>
                <Offcanvas.Header closeButton style={{ background: "rgb(237 237 237 / 76%)", fontWeight: "800", letterSpacing: "1.5px", fontSize: "20px" }}>
                    ...
                    <Offcanvas.Title className="fs-4">DETAILS OF MEMO {showMemo}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
            ...
            {genAdvice == 2 && <CommissionRegistrationAdviceList workerType={workerTypeShort[workerType]} />}
        </>
    );
};

export default CommissionRegistrationGenerateAdvice;
