...
import CommissionEdistAdviceList from "./CommissionEdistAdviceList";

const workerTypeShort = ["cw", "tw"];
const monthFull = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];

const CommissionEdistGenerateAdvice = ({ workerType }) => {
    ...
    const { isLoading, error, data, isFetching } = useQuery(["edist-generate-advice", workerType, searchParams], () => fetcher(`/edist-generate-advice?${searchParams}`));
    ...
    const handleSubmit = (e) => {
        ...
        mutate(
            { url: `/edist-generate-advice-post`, body: { receipt_id: radioSelect, worker_type: workerTypeShort[workerType], data: data } },
            {
                onSuccess(data) {
                    ...
                    setGenAdvice(2);
                    setShowSubmit(false);
                    queryClient.invalidateQueries(["edist-generate-advice", workerType]);
                },
                onError(error) {
                    setShowSubmit(false);
    ...
    const handleShow = async (e) => {
        ...
        newMutate(
            { url: `/edist-view-memo-detail-list/` + memo_id },
            {
                onSuccess(data) {
                    setMemoDetails(data);
                },
    ...
    return (
        <>
            ...
            {genAdvice == 2 && <CommissionEdistAdviceList workerType={workerTypeShort[workerType]} />}
        </>
    );
};

export default CommissionEdistGenerateAdvice;
