...
const CommissionEdistCeoRelease = ({ workerType }) => {
    let total_amount = 0;
    ...
    const { isLoading, error, data } = useQuery(["edist-fund-release-list"], () => fetcher(`/edist-fund-release-list?${searchParams}`));
    ...
    const handleCheckSubmit = (e) => {
        ...
        mutate(
            { url: `/edist-fund-release-post`, body: { receipt_id: checkboxSelect, data: data } },
            {
                onSuccess(data) {
                    ...
                    setCheckboxSelect([]);
                    setShowSubmit(false);
                    queryClient.invalidateQueries(["edist-fund-release-list"]);
                },
                onError(error) {
                    setShowSubmit(false);
    ...
    const handleShow = async (e) => {
        ...
        newMutate(
            { url: `/edist-view-release-detail-list/` + release_id },
            {
                onSuccess(data) {
...
export default CommissionEdistCeoRelease;
