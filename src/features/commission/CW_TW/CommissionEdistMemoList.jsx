...
const CommissionEdistMemoList = ({ workerType }) => {
    const [searchParams, setSearchParams] = useState(`workerType=${workerTypeShort[workerType]}`);
    const { isLoading, error, data } = useQuery(["edist-view-memo-list", workerType, searchParams], () => fetcher(`/edist-view-memo-list?${searchParams}`));
    ...
    const DownloadFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/edist-monthly-payment-memo-excel/" + receipt_id, "Bank Advice.xlsx");
    ...
    const DownloadOrderFile = async (receipt_id) => {
        ...
        try {
            const docData = await downloadFile("/edist-monthly-payment-order-to-rlo-excel/" + receipt_id, "Order To RLO.xlsx");
            if (docData === false) toast.error("Unable to download excel");
...
export default CommissionEdistMemoList;
